...
## Answer 2
```{r}
fb_ads |> group_by(party, incumbency) |>summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE))

party_incumbent_promote = fb_ads |>
filter(office %in% c("State House", "State Senate")) |> group_by(party, incumbency) |>summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE))
