...
```{r}
fb_ads |>
count(incumbency) -> incumbency_table 
incumbency_table |>
  knitr::kable()
```
...
filter(office %in% c("US House", "US Senate")) |> group_by(party, incumbency) |> summarize(promote_prop = mean(ad_tone_promote, na.rm = TRUE))


party_incumbent_promote

knitr::kable(party_incumbent_promote, col.names = c("Party", "Incumbency", "Promote Ads Avg."))
```
The type of candidate that sponsored the most promoting ads on average is the 
...
## Answer 3
```{r}
 fb_ads <- fb_ads |> mutate(impressions_millions = impressions /1000000 )

plot_q3 <- fb_ads |> filter(office == "US House") |> ggplot(data = fb_ads, mapping = aes(x = impressions_millions)) + geom_histogram() 

plot_q3

```

The histogram is very skewed to the left and most of the house candidates did not have more than 10 million ad impressions.
...
```{r}
fb_ads
fb_top_plot <- fb_ads |> filter(office == "US House") |>
  slice_max(order_by = impressions_millions, n = 15) |> mutate (cand_name_last = fct_reorder(cand_name_last, impressions_millions)) |> ggplot(
                                                                                                                                              mapping = aes(x = impressions_millions, y = cand_name_last)) +
  geom_bar(stat = "identity" , mapping = aes(fill = party)) + labs(title = "Top 15 House Candidates Facebook Ad Impressions, 2017-2018" , x = "Facebook Ad Impressions (in millions)", y= "Candidate Last Name") + scale_fill_manual(values = c(D = "blue", R = "red"))
